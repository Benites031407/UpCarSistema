...
router.get('/rooms/:room/clients', requireAuth, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to get room clients' });
  }
});
...
router.post('/dashboard/refresh', requireAuth, async (_req, res) => {
  try {
    ...
    res.json({ message: 'Dashboard metrics refreshed' });
  } catch (error) {
    res.status(500).json({ error: 'Failed to refresh dashboard metrics' });
  }
});
